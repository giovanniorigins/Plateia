	...
	function d () {
		this.$get = function () {
			...
			function d ( a ) {
				...
				a = a || function ( a ) {return a}, this.weightFunc = a, this.heap = []
			}

			return d.prototype.push = function ( b ) {this.heap.push( b ), a( this.heap, this.weightFunc, this.heap.length - 1 )}, d.prototype.peek = function () {return this.heap[0]}, d.prototype.pop = function () {
				...
				for ( var e = this.heap.length, f = 0; e > f; f++ )if ( b.equals( this.heap[f], d ) ) {
				...
				this.heap = []
			}, d.prototype.size = function () {return this.heap.length}, d
		}
	}

	function e () {
		function a ( a, c ) {b.isNumber( a ) ? 0 > a ? c( "must be greater than zero!" ) : c( null ) : c( "must be a number!" )}

		var d, e = function () {return{capacity: Number.MAX_VALUE, maxAge: null, deleteOnExpire: "none", onExpire: null, cacheFlushInterval: null, recycleFreq: 1e3, storageMode: "none", storageImpl: null, verifyIntegrity: !0}};
		this.setCacheDefaults = function ( c ) {
			...
			if ( "capacity"in c && a( c.capacity, function ( a ) {if ( a )throw new Error( f + "capacity: " + a )} ), "deleteOnExpire"in c ) {
			...
			if ( "maxAge"in c && a( c.maxAge, function ( a ) {if ( a )throw new Error( f + "maxAge: " + a )} ), "recycleFreq"in c && a( c.recycleFreq, function ( a ) {if ( a )throw new Error( f + "recycleFreq: " + a )} ), "cacheFlushInterval"in c && a( c.cacheFlushInterval, function ( a ) {if ( a )throw new Error( f + "cacheFlushInterval: " + a )} ), "storageMode"in c ) {
			...
			"$window", "BinaryHeap",
			function ( e, f ) {
				function g ( a ) {return a && b.isNumber( a ) ? a.toString() : a}
				...
				function j ( j, k ) {
					function m ( b ) {
						a( b, function ( a ) {
							...
							for ( A.capacity = b; D.size() > A.capacity; )G.remove( D.peek().key, {verifyIntegrity: !1} )
						} )
					}
					...
					function o ( b ) {
						var c = i( B );
						if ( null === b ) {
							if ( A.maxAge )for ( var d = 0; d < c.length; d++ ) {
								var e = c[d];
							...
							if ( b !== A.maxAge ) {
								...
								for ( var d = (new Date).getTime(), e = 0; e < c.length; e++ ) {
									var f = c[e];
									"maxAge"in B[f] || (C.remove( B[f] ), B[f].expires = B[f].created + A.maxAge, C.push( B[f] ), B[f].expires < d && G.remove( f, {verifyIntegrity: !1} ))
								}
							}
						} )
					}

					function p () {for ( var a = (new Date).getTime(), b = C.peek(); b && b.expires && b.expires < a; )G.remove( b.key, {verifyIntegrity: !1} ), A.onExpire && A.onExpire( b.key, b.value ), b = C.peek()}
					...
					function s ( a, c ) {
						...
						if ( A.storageMode = a, c ) {
							...
							H = c
						} else"localStorage" === A.storageMode ? H = e.localStorage : "sessionStorage" === A.storageMode && (H = e.sessionStorage);
						if ( "none" !== A.storageMode && H )if ( F )for ( d = i( B ), f = 0; f < d.length; f++ )v( d[f] ); else u()
					}
					...
					function u () {
						...
						if ( H.removeItem( E + ".keys" ), a && a.length ) {
							for ( var c = 0; c < a.length; c++ ) {
								var d = b.fromJson( H.getItem( E + ".data." + a[c] ) ) || {}, e = d.maxAge || A.maxAge, f = d.deleteOnExpire || A.deleteOnExpire;
								if ( e && (new Date).getTime() - d.created > e && "aggressive" === f )H.removeItem( E + ".data." + a[c] ); else {
					...
					function v ( a ) {"none" !== A.storageMode && H && (H.setItem( E + ".keys", b.toJson( i( B ) ) ), a && H.setItem( E + ".data." + a, b.toJson( B[a] ) ))}
					...
					function y ( a ) {"none" !== A.storageMode && H && H.setItem( E + ".data." + a, b.toJson( B[a] ) )}
					...
					var A = b.extend( {}, {id: j} ), B = {}, C = new f( function ( a ) {return a.expires} ), D = new f( function ( a ) {return a.accessed} ), E = "angular-cache.caches." + j, F = !1, G = this, H = null;
					k = k || {}, this.put = function ( c, d, e ) {
						...
						if ( e.maxAge && null !== e.maxAge )a( e.maxAge, function ( a ) {if ( a )throw new Error( "AngularCache.put(key, value, options): maxAge: " + a )} ); else {
						...
						if ( a in B ) {
							w( d.verifyIntegrity );
							var h = B[a], i = h.value, j = (new Date).getTime(), k = h.deleteOnExpire || A.deleteOnExpire;
							return D.remove( h ), h.accessed = j, D.push( h ), "passive" === k && "expires"in h && h.expires < j && (this.remove( a, {verifyIntegrity: !1} ), A.onExpire ? A.onExpire( a, h.value, d.onExpire ) : d.onExpire && d.onExpire( a, h.value ), i = c), y( a ), i
						}
					}, this.remove = function ( a, b ) {b = b || {}, w( b.verifyIntegrity ), D.remove( B[a] ), C.remove( B[a] ), "none" !== A.storageMode && H && H.removeItem( E + ".data." + a ), delete B[a], x()}, this.removeAll = function () {z(), D.removeAll(), C.removeAll(), B = {}}, this.removeExpired = function ( a ) {
						a = a || {};
						...
						if ( a ) {
							if ( B[a] ) {
								var c = {created: B[a].created, accessed: B[a].accessed, expires: B[a].expires, maxAge: B[a].maxAge || A.maxAge, deleteOnExpire: B[a].deleteOnExpire || A.deleteOnExpire, isExpired: !1};
								return c.maxAge && (c.isExpired = (new Date).getTime() - c.created > c.maxAge), c
							}
							return B[a]
						}
						return b.extend( {}, A, {size: D && D.size() || 0} )
					}, this.keySet = function () {return h( B )}, this.keys = function () {return i( B )}, this.setOptions = t, t( k, !0, {verifyIntegrity: !1} )
				}
				...
				return k.info = function () {
					for ( var a = i( l ), c = {size: a.length, caches: {}}, e = 0; e < a.length; e++ ) {
						var f = a[e];
						c.caches[f] = l[f].info()
					}
					...
					return l[a]
				}, k.keySet = function () {return h( l )}, k.keys = function () {return i( l )}, k.removeAll = function () {for ( var a = i( l ), b = 0; b < a.length; b++ )l[a[b]].destroy()}, k.clearAll = function () {for ( var a = i( l ), b = 0; b < a.length; b++ )l[a[b]].removeAll()}, k
			}
		]
	}

	b.module( "jmdobry.binary-heap", [
	] ), b.module( "jmdobry.binary-heap" ).provider( "BinaryHeap", d ), b.module( "jmdobry.angular-cache", [
		"ng",
		"jmdobry.binary-heap"
	] ), b.module( "jmdobry.angular-cache" ).provider( "$angularCacheFactory", e )
